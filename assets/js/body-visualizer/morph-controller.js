import {HEIGHT_TARGET,clampHeightInfluence} from './height-calibration.js';
...
export const productionMorphs=Object.freeze([...Object.values(morphPairs).flatMap(p=>[p+'-incr',p+'-decr']),HEIGHT_TARGET]);
...
export function createMorphController(root,{warn=message=>console.warn(message)}={}){
  ...
  function setMorph(name,value){
    ...
    if(name===HEIGHT_TARGET){write(name,clampHeightInfluence(finite(value)));return;}
  ...
  function apply(values){setMorph(HEIGHT_TARGET,values.height??0);for(const key of Object.keys(morphPairs))setBidirectionalMorph(key,values[key]??0);}
