export function mountMorphDebug(root,inspect){
  ...
  const update=()=>{if(!panel.open||document.hidden)return;const {state,inputs,viewer}=inspect();
    output.textContent=JSON.stringify({mode:state.mode,inputs,sources:['qa/body-measurement-calibration/measurements.json','qa/hip-measurement-calibration/measurements.json'],current:viewer?.mappingDebug.current,goal:viewer?.mappingDebug.goal,applied:viewer?.applied,animating:viewer?.animating,note:'Applied = actual mesh influences now; during compare this is the last (goal) scissor pass. Current/goal entries show each pass target.'},null,2);
